...
def data_frame_by_role(role: str, df: pd.DataFrame, zip_codes: list[str]) -> pd.DataFrame:
    return df[
...
def main():
    ...
    unit_len = len(df[df['payment_role'].str.endswith('Unit::Leader')]) +  len(df[df['payment_role'].str.endswith('Unit::Member')])
