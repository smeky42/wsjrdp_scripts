...
def insert_mailing_list(
    ...
    _LOGGER.info(
        "Inserted mailing list %s for unit %s (group id %s) with db_id %s and description %s",
        mail_name,
        name,
        gid,
        new_id,
...
def subscribe_to_mailing_list_sql(mailing_list_id: int, subscriber_id: int, con):
    ...
    """
    cur = con.cursor()
    cur.execute(subscribe_sql, (mailing_list_id, "Group", subscriber_id))
    new_id = cur.fetchone()[0]
    con.commit()
    cur.close()

    _LOGGER.info(
        "Subscribed group id %s to mailing list id %s in db_id %s",
        subscriber_id,
        mailing_list_id,
        new_id,
    )
...
def add_related_role_types_sql(relation_id: int, role_type: str, con):
    ...
    """
    cur = con.cursor()
    cur.execute(subscribe_sql, (relation_id, role_type, "Subscription"))
    new_id = cur.fetchone()[0]
    con.commit()
    cur.close()

    _LOGGER.info(
        "Add related role type %s to relation id %s in db_id %s",
        role_type,
        relation_id,
        new_id,
    )
    ...
    with ctx.psycopg_connect() as conn:
        ...
        for _, row in gdf.iterrows():
            if len(row["name"]) == 2:
                ...
                description = (
                    f"Schreibe an alle Mitglieder (UL und YP) der Unit {row['name']}"
                )
                ...
                for _, urow in uldf.iterrows():
                    if urow["primary_group_id"] == row["group_id"]:
                        additional_sender += (
                            f", {urow['username']}@units.worldscoutjamboree.de"
                        )
                ...
                subcribe_id = subscribe_to_mailing_list_sql(
                    mailing_list_id=mailing_list_id,
                    subscriber_id=row["group_id"],
                    con=conn,
                )
                add_related_role_types_sql(
                    relation_id=subcribe_id, role_type="Group::Unit::Leader", con=conn
                )
                add_related_role_types_sql(
                    relation_id=subcribe_id, role_type="Group::Unit::Member", con=conn
                )
